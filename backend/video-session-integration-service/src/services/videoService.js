...
const generateNeuralLink = async (
  patientId,
  doctorId,
  appointmentId = null,
) => {
  const safeAppointment = appointmentId
...
const getDoctorsForVideo = async (query = {}) => {
  try {
    ...
    console.error(
      "[videoService] failed to fetch doctors from doctor-service:",
      error.message,
    );
    ...
    if (doctorCache.data.length > 0) {
      return {
        ...
        degraded: true,
        message:
          "Doctor service currently unavailable. Showing cached doctor list.",
        cachedAt: doctorCache.lastUpdated,
      };
    }
...
const getDoctorCatalogStatus = async () => {
  try {
    ...
    console.error(
      "[videoService] doctor-service health check failed:",
      error.message,
    );
    return {
      doctorService: "down",
  ...
  return DoctorCatalog.findOneAndUpdate(
    { doctorId: doctorPayload.doctorId },
    {
      ...
      name:
        ...
        `Doctor ${doctorPayload.doctorId}`,
      email: doctorPayload.email || null,
      specialization:
        doctorPayload.specialization || doctorPayload.specialty || null,
