...
const sendSMSViaNotifyLK = async (phoneNumber, message) => {
  ...
  if (!/^\d{11}$/.test(to11)) {
    throw new Error(
      "notify.lk requires 11-digit Sri Lankan number (e.g. 947XXXXXXXX)",
    );
  }
  ...
  for (const formBody of bodyVariants) {
    try {
      ...
      if (data && Array.isArray(data.errors) && data.errors.length) {
        console.warn(
          "notify.lk POST returned errors, will try other variant/fallback",
          data.errors,
        );
        continue;
      }
...
const sendSMSPayload = async ({ phoneNumber, message }) => {
  ...
  if (!normalized) {
    throw new Error(
      "Invalid phone number format. Use +947xxxxxxx or 07xxxxxxx.",
    );
  }
...
const sendSMS = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, provider: SMS_PROVIDER, result });
  } catch (error) {
...
const buildAppointmentNotificationMessages = (payload, routingKey) => {
  const appointmentLabel =
    payload.appointmentType === "ONLINE" ? "online appointment" : "appointment";
...
const processAppointmentNotificationEvent = async (payload, routingKey) => {
  const notification = buildAppointmentNotificationMessages(
    payload,
    routingKey,
  );
  if (!notification) {
  ...
  if (payload.phone) {
    promises.push(
      sendSMSPayload({ phoneNumber: payload.phone, message: notification.sms }),
    );
  }
  ...
  if (errors.length) {
    throw new Error(
      `Appointment notification failed: ${errors.map((err) => err.message || err).join("; ")}`,
    );
  }
...
const processVideoNotificationEvent = async (payload, routingKey) => {
  ...
  if (errors.length) {
    throw new Error(
      `Video session notification failed: ${errors.map((err) => err.message || err).join("; ")}`,
    );
  }
...
const processPaymentNotificationEvent = async (payload, routingKey) => {
  ...
  if (errors.length && successes.length === 0) {
    throw new Error(
      `Payment notification failed: ${errors.map((err) => err.message || err).join("; ")}`,
    );
  }
...
const getEmailTransporter = () => {
  ...
  if (!user || !pass) {
    throw new Error(
      "GMAIL_USER and GMAIL_PASS must be configured for notification email delivery",
    );
  }
...
const sendRegistrationEmail = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: `Registration email sent to ${email}` });
  } catch (error) {
    error.statusCode = error.statusCode || 500;
    next(error);
  }
